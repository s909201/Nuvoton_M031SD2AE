...
void SYS_Init(void)
{
    ...
    CLK->CLKSEL0 = (CLK->CLKSEL0 & (~CLK_CLKSEL0_HCLKSEL_Msk)) | CLK_CLKSEL0_HCLKSEL_HIRC;  // CLK_CLKSEL0_HCLKSEL_HIRC = 0b00000111
    ...
    /* Set PF multi-function pins for UART1 RXD=PF.1 and TXD=PF.0 */
    // SYS->GPF_MFPH &= ~(SYS_GPF_MFPL_PF0MFP_Msk | SYS_GPF_MFPL_PF1MFP_Msk);
    // SYS->GPF_MFPH |= (SYS_GPF_MFPL_PF1MFP_UART0_RXD | SYS_GPF_MFPL_PF0MFP_UART0_TXD);

    /* Set PA multi-function pins for UART0 RXD=PA.15 and TXD=PA.14 */
    SYS->GPA_MFPH &= ~(SYS_GPA_MFPH_PA15MFP_Msk | SYS_GPA_MFPH_PA14MFP_Msk);
